...
- configuration/settings tree access
- authenticated entry into firmware update mode
...
- Settings tree is readable; protected nodes enforce locking
...
- **Static IP** is stored in the device settings tree and applied by the resident firmware.
...
- `nonce_d` (device nonce; used for later authenticated commands)

## Security model (broadcast-safe control plane)

The silicon UID is **not a secret**; it is used as a device descriptor (and may be used as input/salt).

### Control-plane authentication

- Use a **pre-shared secret key (PSK)** (best: per-device, acceptable: per product line).
- Use **challenge-response** to prevent replay:
  - Device provides `nonce_d` in `DISCOVER_REPLY`.
  - Host sends a command with its own `nonce_h` and:
    - `auth = HMAC-SHA256(PSK, uid || nonce_d || nonce_h || cmd || params)`
  - Device verifies HMAC and nonce freshness.
- Hardening:
  - short nonce lifetime
  - optional monotonic counters in metadata (trade-off: flash wear)

### Firmware integrity/authenticity

- Minimum: compute **SHA-256** over the incoming firmware image and verify before marking valid.
- Recommended for field devices: **signed firmware verification** (ECDSA/Ed25519) in the resident firmware; use HMAC only to gate sessions/commands.
...
## Unified settings/item tree (node model)
